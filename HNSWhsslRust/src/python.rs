...
pub fn hnsw_based_dendrogram<'py>(
	...
	insert_minibatch_size: Option<usize>,
	n_rounds: Option<usize>,
) -> (Vec<(usize, usize, f32, usize)>, Vec<f32>, Vec<f32>) {
	let hnsw_params = HNSWParams::new()
	.maybe_with_higher_max_degree(higher_max_degree)
	.maybe_with_lowest_max_degree(lowest_max_degree)
	.maybe_with_max_layers(max_layers)
	...
	;
	unsafe {
		let (mut dendrogram, mut core_distances, milestone_times) = crate::cluster::hnsw_based_dendrogram::<f32, usize, _, _>(
				&arrview2_py_to_rust(data.as_array()),
				SquaredEuclideanDistance::new(),
				min_pts,
		...
		dendrogram.iter_mut().for_each(|(_,_,d,_)| *d = d.max(0.0).sqrt());
		core_distances.iter_mut().for_each(|d| *d = d.max(0.0).sqrt());

		let milestone_seconds = milestone_times
			.into_iter()
			.map(|d| d.as_secs_f32())
			.collect::<Vec<f32>>();

		(dendrogram, core_distances, milestone_seconds)
...
) -> (Vec<(usize, usize, f32, usize)>, Vec<f32>, Vec<Duration>) {
	let hnsw_params = HNSWParams::new()
	.maybe_with_higher_max_degree(higher_max_degree)
	.maybe_with_lowest_max_degree(lowest_max_degree)
	.maybe_with_max_layers(max_layers)
...
#[pyo3(signature = (
    data,
	ef,
    higher_max_degree=None,
    lowest_max_degree=None,
    max_layers=None,
    n_parallel_burnin=None,
    max_build_heap_size=None,
    max_build_frontier_size=None,
    level_norm_param_override=None,
    insert_heuristic=None,
    insert_heuristic_extend=None,
    post_prune_heuristic=None,
    insert_minibatch_size=None,
    n_rounds=None
))]
fn hnsw_hssl(
    data: Bound<'_, PyArray2<f32>>,
	ef: usize,
    higher_max_degree: Option<usize>,
    lowest_max_degree: Option<usize>,
    max_layers: Option<usize>,
    ...
    unsafe {
        let rust_view = arrview2_py_to_rust(data.as_array());

        // build HNSW params just like in graph_based_dendrogram
        let hnsw_params = HNSWParams::new()
            .maybe_with_higher_max_degree(higher_max_degree)
            .maybe_with_lowest_max_degree(lowest_max_degree)
            .maybe_with_max_layers(max_layers)
        ...
        let (dendrogram, milestone_times) =
            crate::cluster::hnsw_hssl::<f32, usize, _, _>(
                &rust_view,
                SquaredEuclideanDistance::new(),
				&ef,
                hnsw_params,
        ...
        // Convert milestone times from Duration to f32
        let milestone_seconds = milestone_times
            .into_iter()
            .map(|d| d.as_secs_f32())
            .collect::<Vec<f32>>();

        // Return as tuple — PyO3 auto-converts Vec to Python list
